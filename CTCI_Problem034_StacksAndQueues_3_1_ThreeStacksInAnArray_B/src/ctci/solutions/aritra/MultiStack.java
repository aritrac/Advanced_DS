 ...
 * 
 * Solutions: This second approach is to allow the stack blocks to be flexible in size. When one stack exceeds its initial capacity, we grow the allowable
 * capacity and shift elements as necessary
 * 
...
public class MultiStack {
	/*
	 * StackInfo is a simple class that holds a set of data about each stack. It does not hold the actual items in the stack. We could have done this with just
	 * a bunch of individual variables, but thats messy and doesn't gain us much
	 */
	...
	private class StackInfo{
		...
		public boolean isWithinStackCapcacity(int index){
	...
	//Shift items in stack over by one element. If we have available capacity, then we'll end up shrinking the stack by one element. If we don't have available 
	//capacity, then we'll need to shift the stack over too
	...
	private void shift(int stackNum){
		...
		while(stack.isWithinStackCapcacity(index)){
	...
	private int adjustIndex(int index){
		//Javas mod operator can return neg values. For example, (-11 % 5) will return -1, not 4. We actually want the value to be 4
