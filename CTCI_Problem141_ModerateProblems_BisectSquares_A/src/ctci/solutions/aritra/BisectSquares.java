...
/*
 * Question: Given two squares on a two-dimensional plane, find a line that would cut these two squares in half. Assume that the top and the bottom sides of the square run parallel to the x-axis.
 * 
 * Solution: Before we start, we should think about what exactly this problem means by a line. Is a line defined by a slope and a y-intercept? Or by any two points on the line? Or,
 * should the line be really a line segment, which starts and ends at the edges of the squares?
 * We will assume, since it makes the problem a bit more interesting, that we mean the third option: that the line should end at the edges of the squares. This line that cuts
 * two squares in half must connect the two middles. We can easily calculate the slope, knowing that slope = (y2-y1)/(x2-x1). Once we calculate the slope using the two middles, we can use the same
...
public class BisectSquares {
	...
	public BisectSquares(double left, double top, double size) {
		this.left = left;
		this.top = top;
	...
	public boolean contains(BisectSquares other) {
